...
Build an English, instruction-only Agent Skill named **SureForge** (`sureforge`) for complex tasks. Its benefit is fewer avoidable errors at the first user-facing delivery through research, explicit decisions, staged work, independent review, and version-bound evidence. The procedure is the owner's established working practice; the published documents state what has been measured and what the next measurement is. The skill is not a runtime enforcement engine and does not promise error-free work.
...
Before any release: resolve material findings, run the authorized behavioral study or obtain an explicit owner decision to publish ahead of it, recheck affected evidence, audit the actual Git metadata, test the intended remote install after authorization, and obtain explicit publication approval. Do not silently relax a release criterion, and do not attach numbers to the release that were not measured. For 1.0.0 the owner-decision path was taken, not the behavioral-study path, so the release cites the pilot figures only; the remote-side steps are recorded in the release notes as described in the publication decision above. Any later release repeats this gate on its own snapshot.
