...
Use an isolated project and an explicitly selected target. The repository test targets Devin's `.devin/skills/sureforge/` and compares every installed file with the source. It does not install into the owner's real global skill directories. The root README documents the local command; remote repository installation is not yet verified and no public repository is assumed.
